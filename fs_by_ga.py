...
def calculate_performance(temp_rank, used_gt_rank, js_dev):
    ...
    used_gt = []
    for each in gt_valid:
        if not np.isnan(each):
            used_gt.append(each)
    used_temp_rank = []
    for each in temp_rank_valid:
        if not np.isnan(each):
            used_temp_rank.append(each)
    cor, p = scipy.stats.kendalltau(used_temp_rank, used_gt, nan_policy="omit")
    return cor
        ...
        for i in range(len(used_feature)):
            if i not in selected_features:
                ...
                if temp_performance > performance:
                    ...
                    add_feature = i
                    is_better = 1
        if not np.isnan(add_feature):
            selected_features.append(add_feature)

        if len(selected_features) > 1:
            remove_feature = np.nan
            for i in range(len(selected_features)):
                temp_features = selected_features.copy()
                temp_remove_feature = temp_features.pop(i)
                temp_rank = calculate_rank(temp_features, used_feature)
                temp_performance = calculate_performance(temp_rank, used_gt_rank, used_feature[-1])
                if temp_performance > performance:
                    performance = temp_performance
                    remove_feature = temp_remove_feature
                    is_better = 1
            if not np.isnan(remove_feature):
